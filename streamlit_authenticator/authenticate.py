...
class Authenticate:
    ...
    def login(self, form_name: str, location: str='main') -> tuple:
        ...
        if not st.session_state['authentication_status']:
            ...
            if st.session_state['authentication_status'] != True:
                ...
                self.username = login_form.text_input('Usuário').lower()
    ...
    def reset_password(self, username: str, form_name: str, location: str='main') -> bool:
        ...
        if location not in ['main', 'sidebar']:
            raise ValueError("Location must be one of 'main' or 'sidebar'")
        if location == 'main':
            reset_password_form = st.form('Resetar senha')
        elif location == 'sidebar':
            reset_password_form = st.sidebar.form('Resetar senha')
        
        reset_password_form.subheader(form_name)
        self.username = username.lower()
        self.password = reset_password_form.text_input('Senha atual', type='password')
        new_password = reset_password_form.text_input('Nova senha', type='password')
        new_password_repeat = reset_password_form.text_input('Repetir senha', type='password')
                ...
                if len(new_password) > 0:
                    if new_password == new_password_repeat:
                        if self.password != new_password: 
                            ...
                            raise ResetError('Nova senha é igual a anterior')
                    else:
                        raise ResetError('As senhas informadas não são iguais')
                else:
                    raise ResetError('Não foi inserida nenhuma senha')
            else:
    ...
    def register_user(self, form_name: str, location: str='main', preauthorization=True) -> bool:
        ...
        if location not in ['main', 'sidebar']:
            raise ValueError("Location must be one of 'main' or 'sidebar'")
        if location == 'main':
            register_user_form = st.form('Registrar usuário')
        elif location == 'sidebar':
            register_user_form = st.sidebar.form('Registrar usuário')

        register_user_form.subheader(form_name)
        new_email = register_user_form.text_input('Email')
        new_username = register_user_form.text_input('Usuário').lower()
        new_name = register_user_form.text_input('Nome')
        new_password = register_user_form.text_input('Senha', type='password')
        new_password_repeat = register_user_form.text_input('Repetir senha', type='password')
                    ...
                    if new_password == new_password_repeat:
                        if preauthorization:
                            if new_email in self.preauthorized['emails']:
                                ...
                                raise RegisterError('Você não tem autorização para cadastrar novos usuários')
                        else:
                            self._register_credentials(new_username, new_name, new_password, new_email, preauthorization)
                            return True
                    else:
                        raise RegisterError('Senhas não combinam')
                else:
                    raise RegisterError('Usuário já existe')
            else:
                raise RegisterError('Por favor informe um email, usuário, nome, e senha')
    ...
    def forgot_password(self, form_name: str, location: str='main') -> tuple:
        ...
        if location not in ['main', 'sidebar']:
            raise ValueError("Location must be one of 'main' or 'sidebar'")
        if location == 'main':
            forgot_password_form = st.form('Esqueci a senha')
        elif location == 'sidebar':
            forgot_password_form = st.sidebar.form('Esqueci a senha')

        forgot_password_form.subheader(form_name)
        username = forgot_password_form.text_input('Usuário').lower()
    ...
    def forgot_username(self, form_name: str, location: str='main') -> tuple:
        ...
        if location not in ['main', 'sidebar']:
            raise ValueError("Location must be one of 'main' or 'sidebar'")
        if location == 'main':
            forgot_username_form = st.form('Esqueci usuário')
        elif location == 'sidebar':
            forgot_username_form = st.sidebar.form('Esqueci usuário')
    ...
    def update_user_details(self, username: str, form_name: str, location: str='main') -> bool:
        ...
        if location not in ['main', 'sidebar']:
            raise ValueError("Location must be one of 'main' or 'sidebar'")
        if location == 'main':
            update_user_details_form = st.form('Alterar dados do usuário')
        elif location == 'sidebar':
            update_user_details_form = st.sidebar.form('Alterar dados do usuário')
        
        update_user_details_form.subheader(form_name)
        self.username = username.lower()
        field = update_user_details_form.selectbox('Campo', ['Name', 'Email']).lower()
        new_value = update_user_details_form.text_input('Novo valor')

        if update_user_details_form.form_submit_button('Atualizar'):
            if len(new_value) > 0:
